...
const Register = () => {
  ...
  const handleUserRegistration = async () => {
    try {
      ...
      const response = await fetch("http://localhost:3000/api/singleuser2", {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        ...
        try {
          ...
          if (userResponse.ok) {
            ...
            // Step 2: Continue with creating the doctor
            handleDoctorCreation(userID);
          } else {
  ...
  const handleDoctorCreation = async (userID) => {
    // Send a POST request to create a new doctor with the acquired UserID
    try {
  ...
  return (
    <div className="mx-auto mt-[10rem] mb-10 bg-[#0F3D3E] p-10 rounded-lg flex flex-col w-[25%]">
      <h1 className="text-2xl text-white font-bold mb-8">Add A Doctor</h1>
